...
cc_dat[!grepl('Unassigned', source) & !grepl('Unassigned', target) & organ == 'Skin' & experiment == 'HDAC2'] %>%
  ...
  ggtitle('Number of Interactions in Skin HDAC2')
ggsave(file.path(plotsDir,'SKinHdac2.pdf'), width = 10)
...
cc_dat[!grepl('Unassigned', source) & !grepl('Unassigned', target) & organ == 'Skin' & experiment == 'HDAC2'][, dcast(.SD, source + pathway_name + organ + experiment ~ treatment, 
           ...
           ][abs(interaction_count_difference) > 15] %>% 
    ggplot() +
  ...
  ggtitle('Skin HDAC2: Source - Differential Interactions')
ggsave(file.path(plotsDir, 'source.pdf'), height = 7, width = 10)
...
cc_dat[!grepl('Unassigned', source) & !grepl('Unassigned', target) & organ == 'Skin' & experiment == 'HDAC2'][, dcast(.SD, target + pathway_name + organ + experiment ~ treatment, 
        ...
        melt(.SD, measure.vars = patterns('vs'),
          ...
          theme_my(panel.background = element_rect(fill = 'white')) +
          scale_fill_gradient2(low = 'blue', mid = 'white', high = 'red') +
        ggtitle('Skin HDAC2: Target - Differential Interactions')
      ggsave(file.path(plotsDir, 'target.pdf'), height = 7, width = 10)
...
cc_analysis <- function(object, pathways, thresh = 0.05){
  ...
  pathway_plots <- sapply(pathways,
    ...
    dev.noerror()
    par(mfrow=c(1,1))
    contributions <- sapply(as.character(unique(object@idents)), function(y){
                            sapply(pathways,
                                 function(x) {
                                   tryCatch(
    ...
    dev.noerror()
    contribution_data <- lapply(contributions, function(x) {
      lapply(x, function(y) y$LR.contribution)
    })
                                
    contribution_data <- rbindlist(lapply(contribution_data, function(l1){
      rbindlist(l1, idcol = 'pathway')
    }), idcol = 'celltype')
                                
    
    contribution_plots <- lapply(contributions, function(x) {
      lapply(x, function(y) y$gg.obj)
    })
...
cc_res <- sapply(cc, function(x) {
  x <- x %>% subsetCellChat(idents.use = c('Unassigned3',
                                      'Unassigned1', 
                                      'Unassigned2'), 
                       invert = TRUE)
    cc_analysis(x,
...
contributions[, c('treatment', 'object') := .(factor(
  str_replace(treatment, '-', '_'), levels = c('NoT', 'HDAC_WT', 'HDAC_cKO')),
  NULL)]
lapply(unique(contributions$celltype), function(x) {
  contributions[celltype == x, by = .(organ, experiment, treatment), 
                .(rank = frank(-contribution, ties.method = 'dense'),
                  pathway, name, contribution)
                ][,
                  name := do.call(paste, .(pathway, name, sep = ': '))
                  ][
                  rank < 10
                  ] %>% 
    ggplot() +
    ...
    scale_fill_manual(values=c("salmon3",
                                "navajowhite1",
                                "palegreen3")) + 
    ggtitle(x, 'Contribution of L-R Pairs to Total Signalling')
  
  ggsave(file.path(plotsDir, paste0('contributions_', x, '.pdf')),
         width = 10, height = 9)
  NULL
})
